...
export default function Home() {
  ...
  const handleSelectPaymentMethod = async (method: 'stars' | 'click') => {
    ...
    // ✅ CRITICAL: For Click, open popup window IMMEDIATELY (synchronously)
    // This prevents popup blockers from interfering
    let paymentWindow: Window | null = null;
    if (method === 'click') {
      paymentWindow = window.open('about:blank', '_blank');
      if (!paymentWindow) {
        showAlertMessage('Popup blocker yoqilgan. Iltimos, brauzer sozlamalarida ruxsat bering.');
        setPaymentLoading(false);
        return;
      }
    }
    ...
    try {
      ...
      if (method === 'stars') {
        ...
        // ✅ Redirect the already-opened window to payment URL
        if (paymentWindow) {
      ...
      // ✅ Close the popup window if Click payment failed
      if (method === 'click' && paymentWindow) {
        paymentWindow.close();
      }
